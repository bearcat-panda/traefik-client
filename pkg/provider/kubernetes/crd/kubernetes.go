...
func (p *Provider) NewK8sClient(ctx context.Context) (*clientWrapper, error) {
	...
	var client *clientWrapper
	switch {
	case len(p.Endpoint) != 0:
		log.FromContext(ctx).Infof("Creating in-cluster Provider client%s", withEndpoint)
		client, err = newInClusterClient(p.Endpoint)
	case len(p.ConfigFile) != 0:
		log.FromContext(ctx).Infof("Creating cluster-external Provider client from KUBECONFIG %s", p.ConfigFile)
		client, err = newExternalClusterClientFromFile(p.ConfigFile)
	case len(p.ConfigBytes) != 0:
		log.FromContext(ctx).Infof("Creating cluster-external Provider client from KUBECONFIG")
		client, err = newExternalClusterClientFromKubeConfig(p.ConfigBytes)
	default:
		log.FromContext(ctx).Infof("Creating cluster-external Provider client%s", withEndpoint)
		client, err = newExternalClusterClient(p.Endpoint, p.Token, p.CertAuthFilePath)
	}
